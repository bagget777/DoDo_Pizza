...
#     cursor.execute('UPDATE users SET phone_number = ? WHERE id_user = ?', (message.text, message.from_user.id))
#     conn.commit()
...
# async def start_command(message: types.Message): print(message.chat.id) 
